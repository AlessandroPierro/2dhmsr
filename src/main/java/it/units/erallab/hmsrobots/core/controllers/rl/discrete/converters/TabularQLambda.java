...
public class TabularQLambda extends TabularSARSALambda {
    public TabularQLambda(double discountFactor, double lambda, int stateDim, int actionDim, double meanQ, double stdQ, int seed) {
    ...
    protected void updateQTable(int newState, double reward, int newAction) {
        ...
        double delta = reward + discountFactor * qTable.getEntry(newState, maxAction) - qTable.getEntry(previousState, previousAction);
